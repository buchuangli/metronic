function brBtn(){
    ...
    var paginationArr=['访问浏览器统计面积图','访问浏览器条形统计图']
...
function BRarea(data){
	...
	var i=data.length-1
	while(i>0||i==0){
		var tr=$("<tr><td>"+(data.length-i)+"</td><td><div class='urlTd' style='width:"+($(".txtData>.countent").width())*0.65+"px"+";''>"+data[i].browserType+"</div></td><td>"+data[i].browserNum+"</td></tr>");
		$("tbody",table).eq(0).append(tr);
		i-=1;
	}
